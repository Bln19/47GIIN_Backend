...
def register_routes(app):
    ...
    #Generar reportes de propietarios de una urbanizacion
    
    @app.route('/report/propietarios/pdf', methods=['GET'])
    @jwt_required()
    def get_propietarios_report_pdf():
        current_user_id = get_jwt_identity()
        connection = get_db_connection()
        cursor = connection.cursor(dictionary=True)
        ...
        # Obtener los propietarios de la urbanización
        query_propietarios = """
            SELECT u.id_perfilUsuario, u.nombre, u.apellidos, u.email, u.telefono
            FROM user u
            WHERE u.id_rol = (SELECT id_rol FROM rol WHERE nombre = 'propietario')
            AND u.id_urbanizacion = %s
            ORDER BY u.id_perfilUsuario ASC
        """
        cursor.execute(query_propietarios, (urbanizacion_id,))
        propietarios = cursor.fetchall()

        cursor.close()
        connection.close()

        pdf_buffer = generar_reporte_propietarios_pdf(nombre_urbanizacion, propietarios)
        return send_file(pdf_buffer, as_attachment=True, attachment_filename='propietarios_reporte.pdf', mimetype='application/pdf')
